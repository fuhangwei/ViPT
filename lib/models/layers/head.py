...
class CenterPredictor(nn.Module, ):
    ...
    def cal_bbox_topk(self, score_map_ctr, size_map, offset_map, k=20, return_score=False):
        score_vec = score_map_ctr.flatten(1)
        k = min(k, score_vec.shape[1])
        top_scores, idx = torch.topk(score_vec, k=k, dim=1)
        idx_y = idx // self.feat_sz
        idx_x = idx % self.feat_sz

        gather_idx = idx.unsqueeze(1).expand(idx.shape[0], 2, idx.shape[1])
        size = size_map.flatten(2).gather(dim=2, index=gather_idx).permute(0, 2, 1)
        offset = offset_map.flatten(2).gather(dim=2, index=gather_idx).permute(0, 2, 1)

        bbox = torch.cat([((idx_x.to(torch.float) + offset[:, :, 0]) / self.feat_sz).unsqueeze(-1),
                          ((idx_y.to(torch.float) + offset[:, :, 1]) / self.feat_sz).unsqueeze(-1),
                          size], dim=-1)

        if return_score:
            return bbox, top_scores
        return bbox
