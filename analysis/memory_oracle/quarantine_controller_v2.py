"""Pure online quarantine controller for approved RMG-Q Stage 1 v2."""
...
class QuarantinePolicy:
    support_iou: float
    immediate_max_entropy: float = 0.45
    quarantine_max_entropy: float = 0.50
    probe_offsets: Tuple[int, ...] = (1, 3, 5)
    min_support: int = 2

    def __post_init__(self) -> None:
        if self.immediate_max_entropy != 0.45:
            raise ValueError("immediate_max_entropy is fixed at 0.45")
...
def triage(observation: Mapping[str, Any], policy: QuarantinePolicy,
    ...
    if entropy <= policy.immediate_max_entropy:
        return ControllerDecision("immediate_update", "entropy_immediate")
    if entropy <= policy.quarantine_max_entropy:
        return ControllerDecision("quarantine", "entropy_quarantine")
...
def admit_opportunity(state: QuarantineState, sequence: str,
    ...
    decision = replace(decision, event_id=event_id, source_frame=frame,
                       effective_frame=(frame + 1 if decision.action == "immediate_update" else None))
