"""Run the approved RMG-Q Stage 1 v3a quarantine protocol.
...
from analysis.memory_oracle.compute_stage1_quarantine_v3a_metrics import (  # noqa: E402
    ARM_NAMES,
...
from analysis.memory_oracle.quarantine_controller_v3a import (  # noqa: E402
    SCHEMA_VERSION,
    QuarantinePolicy,
    QuarantineState,
    admit_opportunity,
...
DEFAULT_CONFIG = Path(__file__).resolve().parent / "configs" / "stage1_quarantine_v3a.yaml"
...
IDENTITY_SOURCES = {
    ...
    "controller": Path(__file__).resolve().parent / "quarantine_controller_v3a.py",
    "runner": Path(__file__).resolve(),
    "metrics": Path(__file__).resolve().parent / "compute_stage1_quarantine_v3a_metrics.py",
...
_FRAME_KEYS = frozenset({
    "schema_version", "sequence", "arm", "frame_idx", "pred_xywh", "best_score",
    "response_entropy", "search_anchor_xywh", "template_id_for_prediction",
    "is_update_opportunity", "action_after_prediction",
})
_PROBE_KEYS = frozenset({
    "frame_idx", "offset", "image_shape", "shared_anchor_xywh", "active_xywh",
    "shadow_xywh", "active_template_id", "shadow_template_id",
    "shadow_search_anchor_xywh", "active_legal", "active_illegal_reason",
    "shadow_legal", "shadow_illegal_reason", "agreement_iou", "supports_release",
})
...
def validate_config(config):
    ...
    if canonical_hash(config) != canonical_hash(default):
        raise ValueError(
            "Stage 1 v3a config differs from the complete frozen default protocol config.")
    if config != default:
        raise ValueError("Stage 1 v3a config is not canonically identical to the frozen default.")
    return config
...
def _require_canonical_jsonl_file(path, expected=None, name="Artifact"):
    path = Path(path)
    rows = read_jsonl(path)
    canonical = "".join(json.dumps(
        row, ensure_ascii=False, sort_keys=True, allow_nan=False) + "\n" for row in rows)
    if path.read_text(encoding="utf-8") != canonical:
        raise RuntimeError("{} is not stored in canonical JSONL byte format: {}".format(
            name, path))
    if expected is not None and (rows != expected
                                 or canonical_hash(rows) != canonical_hash(expected)):
        raise RuntimeError("{} does not canonically equal authoritative regeneration.".format(
            name))
    return rows
...
def _resolve_parent_spec(parent_artifacts):
    ...
    stage2 = parent_artifacts.get("stage1_v2_dir", parent_artifacts.get("stage1_v2"))
    for name, value in (("stage0", stage0), ("stage1_v1", stage1),
                        ("stage1_v2", stage2)):
        if isinstance(value, dict):
            value = value.get("directory", value.get("path"))
        if name == "stage0":
            stage0 = value
        elif name == "stage1_v1":
            stage1 = value
            ...
            stage2 = value
    if not stage0 or not stage1 or not stage2:
        raise ValueError("stage0, stage1_v1, and stage1_v2 parent directories are required.")
    return Path(stage0).resolve(), Path(stage1).resolve(), Path(stage2).resolve()
...
def validate_parent_provenance(parent_artifacts, config=None):
    """Validate Stage 0, Stage 1 v1, and the frozen Stage 1 v2 No-Go."""
    if config is None:
        config = load_config(DEFAULT_CONFIG)
    validate_config(config)
    stage0_dir, stage1_dir, stage2_dir = _resolve_parent_spec(parent_artifacts)
    expected = config.get("parent_artifacts", {})
    stage0_expected = expected.get("stage0", {})
    stage1_expected = expected.get("stage1_v1", {})
    stage2_expected = expected.get("stage1_v2", {})
    ...
    if stage2_expected.get("schema_version") != "rmg-stage1-v2-q5":
        raise RuntimeError("Stage 1 v2 parent schema is not frozen to q5.")
    v2_roots = sorted((stage2_dir / "thresholds").glob("*"))
    expected_grid = [float(value) for value in config["quarantine"][
        "support_iou_candidates"]]
    roots_by_threshold = {}
    root_paths = {}
    for root in v2_roots:
        if not root.is_dir() or not (root / "aggregate.json").is_file() or not (
                root / "gate.json").is_file() or not (root / "metadata.json").is_file():
            continue
        metadata = read_json(root / "metadata.json")
        threshold = float(metadata.get("identity", {}).get("support_iou", -1))
        if threshold in root_paths:
            raise RuntimeError("Stage 1 v2 parent has duplicate threshold roots.")
        root_paths[threshold] = root
    if sorted(root_paths) != expected_grid:
        raise RuntimeError("Stage 1 v2 parent must expose all five evaluated design roots.")

    from analysis.memory_oracle import run_stage1_quarantine_v2 as v2_runner
    v2_config = v2_runner.load_config(v2_runner.DEFAULT_CONFIG)
    v2_runner.validate_config(v2_config)
    v2_parent = v2_runner.validate_parent_provenance({
        "stage0_dir": stage0_dir, "stage1_v1_dir": stage1_dir}, v2_config)
    first_metadata = read_json(root_paths[expected_grid[0]] / "metadata.json")
    design_sequences = first_metadata.get("identity", {}).get("sequences", [])
    if (not isinstance(design_sequences, list) or len(design_sequences) != 83
            or canonical_hash(design_sequences)
            != stage2_expected.get("design_sequences_hash")):
        raise RuntimeError("Stage 1 v2 roots do not bind the frozen canonical design83.")
    aggregate_candidates = []
    stability_input = {sequence: [] for sequence in design_sequences}
    for threshold in expected_grid:
        root = root_paths[threshold]
        validated_v2 = v2_runner.validate_evaluated_root(
            root, "design83", design_sequences, threshold, v2_config,
            v2_parent["content_hash"], policy_hash=None)
        aggregate_v2 = validated_v2["aggregate"]
        aggregate_candidates.append({
            "support_threshold": threshold,
            "governance": aggregate_v2["governance"],
            "quarantine_incremental_success_auc_delta": aggregate_v2[
                "frame_weighted_paired_deltas"]["rmg_q_vs_rmg_q_no_quarantine"][
                    "success_auc"]["mean"],
        })
        for sequence, summary in zip(design_sequences, validated_v2["summaries"]):
            stability_input[sequence].append({
                "support_threshold": threshold,
        ...
        roots_by_threshold[threshold] = {
            "root": str(root.resolve()),
            "source_identity_hash": validated_v2["metadata"]["identity_hash"],
            "aggregate_sha256": file_sha256(root / "aggregate.json"),
            "gate_sha256": file_sha256(root / "gate.json"),
            "aggregate_content_hash": aggregate_v2["content_hash"],
            "gate_content_hash": validated_v2["gate"]["content_hash"],
        }
    recomputed_selection = v2_runner.select_smallest_passing_support_threshold(
        aggregate_candidates, gate_config=v2_config["action_gate"])
    stability_rows = [{"sequence": sequence,
                       "threshold_candidates": stability_input[sequence]}
                      for sequence in design_sequences]
    recomputed_stability = v2_runner.deterministic_threshold_stability(
        stability_rows, gate_config=v2_config["action_gate"],
        seed=v2_config["stability"]["bootstrap_seed"],
        samples=v2_config["stability"]["bootstrap_samples"],
        hash_namespace="rmg-stage1-v2-q5-threshold-stability")
    if (recomputed_selection["selected_support_threshold"]
            is not stage2_expected.get("selected_support_threshold")
            or recomputed_selection["content_hash"]
            != stage2_expected.get("recomputed_selection_content_hash")
            or recomputed_stability["content_hash"]
            != stage2_expected.get("recomputed_stability_content_hash")):
        raise RuntimeError("Stage 1 v2 No-Go is not the frozen authoritative recomputation.")
    forbidden_v2 = [stage2_dir / name for name in (
        "provisional_policy_lock.json", "provisional_policy_lock_v2.json",
        "final_policy_lock.json", "final_policy_lock_v2.json", "internal42",
        "confirm62", "val47", "val47_unlock.json", "val47_unlock_v2.json")
                    if (stage2_dir / name).exists()]
    if stage2_expected.get("lock_must_be_absent") and forbidden_v2:
        raise RuntimeError("Forbidden Stage 1 v2 post-No-Go artifacts exist: "
                           + ", ".join(map(str, forbidden_v2)))
    result = {
        ...
        "stage1_v2": {
            "selected_support_threshold": recomputed_selection[
                "selected_support_threshold"],
            "recomputed_selection_content_hash": recomputed_selection["content_hash"],
            "recomputed_stability_content_hash": recomputed_stability["content_hash"],
            "design_roots": [roots_by_threshold[value] for value in expected_grid],
            "forbidden_artifacts_absent": True,
        },
    }
...
def validate_policy_lock(path, expected_parent_hash=None, required_kind=None,
    ...
    path = Path(path)
    if not path.is_file():
        raise FileNotFoundError("A frozen Stage 1 v3a policy lock is required: {}".format(path))
    ...
    approved_kinds = {"provisional_stage1_quarantine_v3a_policy",
                      "final_stage1_quarantine_v3a_policy"}
    if (not _content_hash_valid(lock) or lock.get("schema_version") != SCHEMA_VERSION
            or lock.get("kind") not in approved_kinds):
        raise RuntimeError("Stage 1 v3a policy lock is corrupt or has the wrong kind/schema.")
    ...
    expected_keys = {
        ...
        "protocol_manifest", "feature_schema_hash", "selection_audit",
        "bootstrap_stability", "logo_stability", "config_hash",
        "parent_provenance_hash", "source_identity_hashes", "source_sha256",
        "confirmation_outcomes_accessed", "content_hash",
    }
    if lock["kind"] == "final_stage1_quarantine_v3a_policy":
    ...
    manifest = _validate_artifact_reference(
        lock.get("protocol_manifest"), "rmg_stage1_v3a_protocol_manifest")
    selection_audit = _validate_artifact_reference(
        lock.get("selection_audit"), "rmg_stage1_v3a_selection_audit")
    bootstrap_audit = _validate_artifact_reference(
        lock.get("bootstrap_stability"), "rmg_stage1_v3a_bootstrap_stability")
    logo_audit = _validate_artifact_reference(
        lock.get("logo_stability"), "rmg_stage1_v3a_logo_stability")
    if (manifest.get("pass") is not True or selection_audit.get("pass") is not True
            or bootstrap_audit.get("pass") is not True or logo_audit.get("pass") is not True
            or manifest.get("feature_schema_hash") != lock.get("feature_schema_hash")
            or manifest.get("selection_audit_content_hash")
            != selection_audit.get("content_hash")
            or manifest.get("bootstrap_stability_content_hash")
            != bootstrap_audit.get("content_hash")
            or manifest.get("logo_stability_content_hash") != logo_audit.get("content_hash")):
        raise RuntimeError("Policy lock audit/manifest chain is not passing or bound.")
    ...
    for index, item in enumerate(candidate_locks):
        ...
        aggregate_candidate = dict(aggregate)
        aggregate_candidate["support_threshold"] = item["support_iou"]
        ...
        for sequence, summary in zip(design_sequences, validated["summaries"]):
            sequence_candidates[sequence].append({
                ...
                "governance": summary["governance"],
                "auc_deltas": {
                    baseline: summary["arms"]["rmg_qh_qonly"]["success_auc"]
                    - summary["arms"][baseline]["success_auc"]
                    for baseline in ("static", "periodic_pred", "rmg_qh_noq")
                },
                "clean_subset": summary["arms"]["static"]["success_auc"] >= 0.60,
            })
    expected_selection_gate = select_smallest_passing_support_threshold(
        aggregate_candidates, gate_config=config["action_gate"],
        final_gate_config=config["final_tracking_gate"])
    ...
    if (selection["selection_gate"] != expected_selection_gate
            ...
            or expected_stability["bootstrap"]["selected_threshold_match_fraction"]
            < stability_config["selected_threshold_match_fraction_min"]
            or expected_stability[
                "leave_one_group_out_same_non_none_threshold_count"]
            < stability_config["leave_one_group_out"][
                "same_non_none_threshold_min"]):
    ...
    if lock["kind"] == "final_stage1_quarantine_v3a_policy":
        ...
        provisional = validate_policy_lock(
            ...
            required_kind="provisional_stage1_quarantine_v3a_policy",
...
def validate_val47_unlock(path, expected_policy_hash=None, expected_parent_hash=None,
    ...
    if (not _content_hash_valid(unlock) or unlock.get("schema_version") != SCHEMA_VERSION
            or unlock.get("kind") != "rmg_stage1_v3a_val47_unlock"):
    ...
    policy = validate_policy_lock(
        ...
        required_kind="final_stage1_quarantine_v3a_policy",
...
def _frame_row(sequence, arm, frame_idx, output, action="none", opportunity=False):
    ...
    row = {
        ...
        "response_entropy": (float(output["response_entropy"])
                             if output.get("response_entropy") is not None
                             and math.isfinite(float(output["response_entropy"])) else None),
...
def _event_row(sequence, observation, decision, legal, invalid_reason):
    ...
    if decision.action not in {"skip", "quarantine", "release", "discard"}:
        raise RuntimeError("v3a forbids unsupported governance action: {}".format(
            decision.action))
    row = {
        "schema_version": SCHEMA_VERSION,
        ...
        "source_search_anchor_xywh": list(observation["search_anchor_xywh"]),
        "source_image_shape": [int(value) for value in observation["image_shape"][:2]],
        "admission_entropy": (
            float(observation["response_entropy"])
            if math.isfinite(float(observation["response_entropy"])) else None),
        "legal_source_opportunity": legal,
        "candidate_invalid_reason": invalid_reason,
        "action": decision.action,
        "reason": decision.reason,
        "admitted": decision.action == "quarantine",
...
def run_arm_online(sequence, arm, rgb_imgs, tir_imgs, init_box, num_frames,
    ...
    for frame_idx in range(1, num_frames):
        ...
        due = due_probe_offsets(qstate, frame_idx, policy) if arm == "rmg_qh_qonly" else ()
        if due:
            ...
            event_by_id[event_id]["probes"].append({
                ...
                "active_template_id": output.get("template_id"),
                "shadow_template_id": shadow.get("template_id"),
                "shadow_search_anchor_xywh": list(shadow.get(
                    "search_anchor", shadow.get("state_before", pre_frame_anchor))[:4]),
        ...
        if opportunity and arm != "static":
            ...
            if arm == "periodic_pred":
            ...
            elif arm in ("rmg_qh_qonly", "rmg_qh_noq"):
                if arm == "rmg_qh_qonly":
                    ...
                    temporary, decision = admit_opportunity(
                        QuarantineState(), sequence, observation, policy, legal,
                        invalid_reason)
                    del temporary
                    if decision.action == "quarantine":
                        decision = type(decision)(
                            "discard", "quarantine_disabled", decision.event_id,
                            decision.source_frame, None)
                event = _event_row(sequence, observation, decision, legal, invalid_reason)
                if arm == "rmg_qh_noq" and decision.action == "discard":
                    event["admitted"] = True
                ...
                if decision.action == "quarantine":
                    ...
                    action = "discard:quarantine_disabled"
                else:
...
def validate_online_trace(rows, sequence, arm):
    ...
    if any(set(row) != _FRAME_KEYS for row in rows):
        raise RuntimeError("Frame trace does not match the exact label-free schema.")
    ...
    for row in rows:
        if (not _exact_box(row.get("pred_xywh"), row.get("pred_xywh"))
                or not _exact_box(row.get("search_anchor_xywh"),
                                  row.get("search_anchor_xywh"))
                or not isinstance(row.get("template_id_for_prediction"), str)
                or not row.get("template_id_for_prediction")
                or not isinstance(row.get("is_update_opportunity"), bool)
                or not isinstance(row.get("action_after_prediction"), str)):
            raise RuntimeError("Frame trace contains invalid prediction/context fields.")
        for key in ("best_score", "response_entropy"):
            value = row.get(key)
            if value is not None and (isinstance(value, bool)
                                      or not isinstance(value, (int, float))
                                      or not math.isfinite(float(value))):
                raise RuntimeError("Frame trace contains a non-finite numeric field.")
    _reject_labels(rows)


def prediction_trace_payload(rows):
    return [{"frame_idx": int(row["frame_idx"]),
             "pred_xywh": [float(value) for value in row["pred_xywh"]]}
            for row in rows]


def validate_static_noq_equivalence(static_rows, noq_rows):
    static_payload = prediction_trace_payload(static_rows)
    noq_payload = prediction_trace_payload(noq_rows)
    if static_payload != noq_payload or canonical_hash(static_payload) != canonical_hash(noq_payload):
        raise RuntimeError("static and rmg_qh_noq predictions are not numerically equivalent.")
    return canonical_hash(static_payload)


def _exact_box(value, expected):
    try:
        return ([float(item) for item in value] == [float(item) for item in expected]
                and len(value) == 4 and len(expected) == 4)
    except (TypeError, ValueError, OverflowError):
        return False


def _canonical_template_id(source, source_frame, box):
    return "{}:{}:".format(source, int(source_frame)) + ",".join(
        "{:.6f}".format(float(value)) for value in box[:4])


def _decode_image_shape(path):
    """Decode an authoritative image for semantic replay, never trusting artifacts."""
    try:
        import cv2
        image = cv2.imread(str(path), cv2.IMREAD_UNCHANGED)
        if image is None or image.ndim < 2:
            raise RuntimeError("OpenCV could not decode authoritative image: {}".format(path))
        ...
        try:
            ...
            raise RuntimeError(
                "Semantic replay requires cv2 or Pillow to decode authoritative images.") from exc
        with Image.open(str(path)) as image:
            width, height = image.size
        return [int(height), int(width)]


def _authoritative_scheduled_shapes(dataset_root, sequence, frame_indices):
    rgb, tir, _ = _load_evaluation_sequence(dataset_root, sequence)
    shapes = {}
    for frame_idx in sorted(set(int(value) for value in frame_indices)):
        if frame_idx < 0 or frame_idx >= len(rgb):
            raise RuntimeError("Scheduled/probe frame lies outside authoritative images.")
        rgb_shape = _decode_image_shape(rgb[frame_idx])
        tir_shape = _decode_image_shape(tir[frame_idx])
        if rgb_shape != tir_shape:
            raise RuntimeError("Visible/infrared dimensions differ at {}/{}.".format(
                sequence, frame_idx))
        shapes[frame_idx] = rgb_shape
    return shapes


def _strict_event_keys(event, finalized):
    base = {
        "schema_version", "event_id", "sequence", "source_frame",
        "source_candidate_xywh", "source_search_anchor_xywh", "source_image_shape",
        "admission_entropy", "legal_source_opportunity", "candidate_invalid_reason",
        "action", "reason", "admitted", "quarantined", "immediate_write",
        "effective_frame", "probes",
    }
    if finalized:
        base |= {"support_count", "probe_count", "finalized_frame", "released"}
    return base


def validate_event_semantic_replay(sequence, arm, rows, events, config, support_iou,
                                   dataset_root=None, image_shapes=None):
    """Replay frozen governance semantics from traces without rerunning the model."""
    validate_online_trace(rows, sequence, arm)
    if arm not in ARMS:
        raise RuntimeError("Unknown arm during semantic replay: {}".format(arm))
    if not isinstance(events, list):
        raise RuntimeError("Online events must be a list.")
    _reject_labels(events)
    schedule = config["schedule"]
    opportunities = periodic_candidate_frames(
        len(rows), schedule["warmup"], schedule["interval"], schedule["cooldown"])
    opportunity_set = set(opportunities)
    if [bool(row.get("is_update_opportunity")) for row in rows] != [
            index in opportunity_set for index in range(len(rows))]:
        raise RuntimeError("Frame opportunity metadata differs from the frozen schedule.")
    if arm in ("static", "periodic_pred") and events:
        raise RuntimeError("static and periodic_pred event lists must be empty.")
    if arm in ("rmg_qh_qonly", "rmg_qh_noq"):
        if len(events) != len(opportunities):
            raise RuntimeError("Governance must contain exactly one event per scheduled opportunity.")
        if [event.get("source_frame") for event in events] != opportunities:
            raise RuntimeError("Governance event source frames omit or reorder opportunities.")

    required_frames = set(opportunities)
    if arm == "rmg_qh_qonly":
        for source in opportunities:
            required_frames.update(source + offset for offset in (1, 3, 5))
    if image_shapes is None:
        if dataset_root is None:
            raise RuntimeError("Strict semantic replay requires authoritative image shapes.")
        image_shapes = _authoritative_scheduled_shapes(
            dataset_root, sequence, [frame for frame in required_frames if frame < len(rows)])
    normalized_shapes = {int(key): [int(value) for value in shape[:2]]
                         for key, shape in image_shapes.items()}
    missing_shapes = sorted(frame for frame in required_frames
                            if frame < len(rows) and frame not in normalized_shapes)
    if missing_shapes:
        raise RuntimeError("Authoritative image shapes are missing replay frames: {}".format(
            missing_shapes))

    if not _exact_box(rows[0].get("search_anchor_xywh"), rows[0].get("pred_xywh")):
        raise RuntimeError("Initial frame search anchor must equal the initialization box.")
    for frame_idx in range(1, len(rows)):
        if not _exact_box(rows[frame_idx].get("search_anchor_xywh"),
                          rows[frame_idx - 1].get("pred_xywh")):
            raise RuntimeError("Frame search anchor does not follow the active prediction trajectory.")

    threshold = float(support_iou)
    policy = QuarantinePolicy(support_iou=threshold)
    initial_template = rows[0].get("template_id_for_prediction")
    if not isinstance(initial_template, str) or not initial_template:
        raise RuntimeError("Initial active template id is missing.")
    expected_actions = {frame: "none" for frame in range(len(rows))}
    if arm in ("static", "periodic_pred"):
        template_updates = {}
        if arm == "periodic_pred":
            for frame_idx in opportunities:
                legal, invalid_reason = _bbox_validity(
                    rows[frame_idx].get("pred_xywh"), normalized_shapes[frame_idx], config)
                if legal:
                    expected_actions[frame_idx] = "commit_current_arm_prediction"
                    template_updates[frame_idx + 1] = _canonical_template_id(
                        "current_arm_prediction", frame_idx, rows[frame_idx]["pred_xywh"])
                else:
                    expected_actions[frame_idx] = "skip:invalid:{}".format(invalid_reason)
        active_template = initial_template
        for frame_idx, row in enumerate(rows):
            if frame_idx in template_updates:
                active_template = template_updates[frame_idx]
            if row.get("template_id_for_prediction") != active_template:
                raise RuntimeError("Baseline frame trace contains an invalid template transition.")
            if row.get("action_after_prediction") != expected_actions[frame_idx]:
                raise RuntimeError("Baseline frame action metadata differs from semantic replay.")
        return canonical_hash(events)

    events_by_source = {int(event["source_frame"]): event for event in events}
    pending_until = None
    release_at = {}

    for frame_idx in range(len(rows)):
        row = rows[frame_idx]
        if frame_idx not in opportunity_set:
            continue
        event = events_by_source[frame_idx]
        if (event.get("schema_version") != SCHEMA_VERSION
                or event.get("sequence") != sequence
                or event.get("event_id") != deterministic_event_id(sequence, frame_idx)):
            raise RuntimeError("Governance event identity is not deterministic/canonical.")
        if event.get("immediate_write") is not False or any(
                "immediate" in str(key).lower() and key != "immediate_write" for key in event):
            raise RuntimeError("v3a event contains immediate-write fields or actions.")
        source_row = rows[frame_idx]
        shape = normalized_shapes[frame_idx]
        legal, invalid_reason = _bbox_validity(source_row.get("pred_xywh"), shape, config)
        entropy = event.get("admission_entropy")
        try:
            entropy_value = float(entropy) if entropy is not None else None
        except (TypeError, ValueError, OverflowError):
            entropy_value = None
        if entropy_value is not None and not math.isfinite(entropy_value):
            entropy_value = None
        if (not _exact_box(event.get("source_candidate_xywh"), source_row.get("pred_xywh"))
                or not _exact_box(event.get("source_search_anchor_xywh"),
                                  source_row.get("search_anchor_xywh"))
                or entropy_value != source_row.get("response_entropy")
                or event.get("source_image_shape") != shape
                or event.get("legal_source_opportunity") is not legal
                or event.get("candidate_invalid_reason") != invalid_reason):
            raise RuntimeError("Governance source evidence differs from authoritative trace/shape replay.")
        if entropy_value != event.get("admission_entropy"):
            raise RuntimeError("Admission entropy must be finite JSON number or null.")

        eligible = legal and entropy_value is not None and entropy_value <= 0.50
        # The controller finalizes before processing a same-frame opportunity at t+5.
        slot_occupied = (arm == "rmg_qh_qonly" and pending_until is not None
                         and frame_idx < pending_until)
        if slot_occupied:
            expected = ("skip", "quarantine_slot_occupied", False, False)
        elif not legal:
            expected = ("skip", "invalid:{}".format(invalid_reason or "unspecified"), False, False)
        elif entropy_value is None:
            expected = ("skip", "non_finite_entropy", False, False)
        elif entropy_value > 0.50:
            expected = ("skip", "entropy_above_quarantine", False, False)
        elif arm == "rmg_qh_noq":
            expected = ("discard", "quarantine_disabled", True, False)
        else:
            expected = None
        ...
        if arm == "rmg_qh_noq":
            if expected is None:
                expected = ("discard", "quarantine_disabled", True, False)
            action, reason, admitted, quarantined = expected
            if (set(event) != _strict_event_keys(event, False)
                    or (event.get("action"), event.get("reason"), event.get("admitted"),
                        event.get("quarantined")) != expected
                    or event.get("effective_frame") is not None or event.get("probes") != []):
                raise RuntimeError("noq event does not exactly implement discard/disabled semantics.")
            expected_actions[frame_idx] = ("discard:quarantine_disabled" if admitted
                                           else "skip:" + reason)
            continue
        ...
        if expected is not None:
            if (set(event) != _strict_event_keys(event, False)
                    or (event.get("action"), event.get("reason"), event.get("admitted"),
                        event.get("quarantined")) != expected
                    or event.get("effective_frame") is not None or event.get("probes") != []):
                raise RuntimeError("qonly skipped event differs from deterministic replay.")
            expected_actions[frame_idx] = "skip:" + expected[1]
            continue

        if not eligible:
            raise RuntimeError("Internal admission replay inconsistency.")
        probes = event.get("probes")
        if (set(event) != _strict_event_keys(event, True) or not isinstance(probes, list)
                or len(probes) != 3):
            raise RuntimeError("Admitted qonly event must contain one complete Q5 replay.")
        if event.get("admitted") is not True or event.get("quarantined") is not True:
            raise RuntimeError("Eligible qonly candidate was not quarantine-admitted.")
        expected_actions[frame_idx] = "quarantine_source_snapshot"
        pending_until = frame_idx + policy.probe_offsets[-1]
        source_template = _canonical_template_id(
            "quarantine_source_prediction", frame_idx, source_row["pred_xywh"])
        support_count = 0
        for probe, offset in zip(probes, policy.probe_offsets):
            probe_frame = frame_idx + offset
            probe_row = rows[probe_frame]
            probe_shape = normalized_shapes[probe_frame]
            active_legal, active_reason = _bbox_validity(
                probe_row.get("pred_xywh"), probe_shape, config)
            shadow_box = probe.get("shadow_xywh")
            shadow_legal, shadow_reason = _bbox_validity(shadow_box, probe_shape, config)
            agreement = iou_xywh(probe_row.get("pred_xywh"), shadow_box)
            agreement = agreement if math.isfinite(agreement) else None
            supports = bool(active_legal and shadow_legal and agreement is not None
                            and agreement >= threshold)
            if (set(probe) != _PROBE_KEYS or probe.get("frame_idx") != probe_frame
                    or probe.get("offset") != offset or probe.get("image_shape") != probe_shape
                    or not _exact_box(probe.get("active_xywh"), probe_row.get("pred_xywh"))
                    or probe.get("active_template_id") != probe_row.get(
                        "template_id_for_prediction")
                    or probe.get("shadow_template_id") != source_template
                    or not _exact_box(probe.get("shadow_search_anchor_xywh"),
                                      probe_row.get("search_anchor_xywh"))
                    or not _exact_box(probe.get("shared_anchor_xywh"),
                                      probe_row.get("search_anchor_xywh"))
                    or probe.get("active_legal") is not active_legal
                    or probe.get("active_illegal_reason") != active_reason
                    or probe.get("shadow_legal") is not shadow_legal
                    or probe.get("shadow_illegal_reason") != shadow_reason
                    or probe.get("agreement_iou") != agreement
                    or probe.get("supports_release") is not supports):
                raise RuntimeError("Probe evidence differs from strict shape/trace/support replay.")
            support_count += int(supports)
        release = support_count >= policy.min_support
        finalized_frame = frame_idx + 5
        effective_frame = frame_idx + 6 if release else None
        expected_final = ("release" if release else "discard",
                          "probe_support_pass" if release else "insufficient_probe_support")
        if ((event.get("action"), event.get("reason")) != expected_final
                or event.get("support_count") != support_count
                or event.get("probe_count") != 3
                or event.get("finalized_frame") != finalized_frame
                or event.get("effective_frame") != effective_frame
                or event.get("released") is not release):
            raise RuntimeError("Q5 finalization/action/effective timing differs from replay.")
        expected_actions[finalized_frame] = (
            "release_source_snapshot" if release else "discard_quarantine")
        if release:
            release_at[effective_frame] = source_template

    active_template = rows[0].get("template_id_for_prediction")
    for frame_idx, row in enumerate(rows):
        if frame_idx in release_at:
            active_template = release_at[frame_idx]
        if row.get("template_id_for_prediction") != active_template:
            raise RuntimeError("Frame trace contains an immediate/late template write.")
        if row.get("action_after_prediction") != expected_actions[frame_idx]:
            raise RuntimeError("Frame action metadata differs from semantic replay.")
    return canonical_hash(events)
...
def _online_sequence_worker(sequence):
    ...
    for arm in ARMS:
        ...
        validate_event_semantic_replay(
            sequence, arm, rows, events, _WORKER["config"], _WORKER["support_iou"],
            dataset_root=_WORKER["dataset_root"])
    ...
    static_rows = read_jsonl(arms_dir / "static.frames.jsonl")
    noq_rows = read_jsonl(arms_dir / "rmg_qh_noq.frames.jsonl")
    equivalence_hash = validate_static_noq_equivalence(static_rows, noq_rows)
    summary = {
        ...
        "static_noq_prediction_hash": equivalence_hash,
    }
...
def completed_online_sequence(output_dir, sequence, support_iou, dataset_root=None,
                              config=None, image_shapes=None):
    ...
    try:
        ...
        if (set(summary) != {"schema_version", "kind", "sequence", "support_iou",
                             "arms", "trace_hashes", "event_hashes",
                             "static_noq_prediction_hash", "content_hash"}
        ...
        for arm in ARMS:
            ...
            if config is not None:
                validate_event_semantic_replay(
                    sequence, arm, rows, events, config, support_iou,
                    dataset_root=dataset_root, image_shapes=image_shapes)
            if summary["trace_hashes"].get(arm) != canonical_hash(rows):
                return False
            if summary["event_hashes"].get(arm) != canonical_hash(events):
                return False
        return summary["static_noq_prediction_hash"] == validate_static_noq_equivalence(
            arm_rows["static"], arm_rows["rmg_qh_noq"])
...
def map_online_sequences(sequences, args, config, support_iou):
    pending = [sequence for sequence in sequences if not completed_online_sequence(
        args.output_dir, sequence, support_iou, dataset_root=args.dataset_root,
        config=config)]
    if not pending:
        return []
    payload = {
...
def _evaluate_sequence_worker(sequence):
    ...
    if not completed_online_sequence(
            _WORKER["output_dir"], sequence, _WORKER["support_iou"],
            dataset_root=_WORKER["dataset_root"], config=_WORKER["config"]):
        raise RuntimeError("Strict online semantic replay failed before evaluation.")
    ...
    for arm in ARMS:
        ...
        trace_hashes[arm] = canonical_hash(rows)
    events = read_jsonl(root / "online" / "rmg_qh_qonly.events.jsonl")
    event_labels = generate_event_labels(events, gt)
    write_jsonl(labels_dir / "rmg_qh_qonly.events.jsonl", event_labels)
    ...
    summary = {
        ...
        "online_rmg_qh_qonly_events_hash": canonical_hash(events),
...
def completed_evaluated_sequence(output_dir, sequence, support_iou=None,
                                 dataset_root=None, config=None, authoritative=False):
    root = safe_sequence_dir(output_dir, sequence)
    ...
    try:
        if support_iou is not None and not completed_online_sequence(
                output_dir, sequence, support_iou, dataset_root=dataset_root,
                config=config):
            return False
        summary = read_json(path)
        ...
        event_labels = read_jsonl(root / "labels" / "rmg_qh_qonly.events.jsonl")
        ...
        events = read_jsonl(root / "online" / "rmg_qh_qonly.events.jsonl")
        _reject_labels(events)
        if summary.get("online_rmg_qh_qonly_events_hash") != canonical_hash(events):
            return False
        if authoritative:
            if dataset_root is None or config is None or support_iou is None:
                raise RuntimeError("Authoritative completion requires dataset_root/config/support_iou.")
            expected = _recompute_authoritative_sequence_summary(
                output_dir, sequence, config, support_iou, dataset_root,
...
def map_evaluate_sequences(sequences, args, config):
    pending = [sequence for sequence in sequences
               if not completed_evaluated_sequence(
                   args.output_dir, sequence, args.support_iou,
                   dataset_root=args.dataset_root, config=config, authoritative=True)]
...
_SUMMARY_KEYS = {
    ...
    "online_trace_hashes", "online_rmg_qh_qonly_events_hash", "frame_labels_hash",
...
def validate_metadata(metadata_path, scope, sequences, support_iou, config,
    ...
    if (metadata.get("schema_version") != SCHEMA_VERSION
            or metadata.get("kind") != "stage1_quarantine_v3a_run_metadata"
    ...
    dataset_root = Path(identity.get("dataset_root", ""))
    if not dataset_root.is_absolute() or not dataset_root.is_dir():
        raise RuntimeError("Experiment identity dataset_root must be absolute and existing.")
    recomputed_manifest = _dataset_manifest_hash(dataset_root, expected_sequences)
    if identity.get("image_manifest_hash") != recomputed_manifest:
        raise RuntimeError("Authoritative dataset/GT manifest no longer matches run identity.")
...
def _recompute_authoritative_sequence_summary(root, sequence, config, support_iou,
                                             dataset_root, require_stored_labels=True):
    """Regenerate labels from sealed GT and metrics from strict raw online traces."""
    sequence_root = safe_sequence_dir(root, sequence, create=False)
    _, _, gt = _load_evaluation_sequence(dataset_root, sequence)
    frame_labels = generate_frame_labels(sequence, gt)
    stored_frame_labels = _require_canonical_jsonl_file(
        sequence_root / "labels" / "frames.jsonl", name="Stored frame labels")
    if require_stored_labels and (stored_frame_labels != frame_labels
                                  or canonical_hash(stored_frame_labels) != canonical_hash(frame_labels)):
        raise RuntimeError("Stored frame labels differ from authoritative GT regeneration.")
    arm_metrics = {}
    trace_hashes = {}
    arm_rows = {}
    for arm in ARMS:
        rows = read_jsonl(sequence_root / "online" / (arm + ".frames.jsonl"))
        events = read_jsonl(sequence_root / "online" / (arm + ".events.jsonl"))
        validate_event_semantic_replay(
            sequence, arm, rows, events, config, support_iou,
            dataset_root=dataset_root)
        if len(rows) != len(frame_labels):
            raise RuntimeError("Online trace length differs from authoritative GT length.")
        arm_rows[arm] = rows
        arm_metrics[arm] = tracking_metrics_from_online_trace(rows, frame_labels)
        _exact_keys(arm_metrics[arm], _ARM_METRIC_KEYS,
                    "{} tracking metrics".format(arm))
        trace_hashes[arm] = canonical_hash(rows)
    validate_static_noq_equivalence(arm_rows["static"], arm_rows["rmg_qh_noq"])
    events = read_jsonl(sequence_root / "online" / "rmg_qh_qonly.events.jsonl")
    event_labels = generate_event_labels(events, gt)
    stored_event_labels = _require_canonical_jsonl_file(
        sequence_root / "labels" / "rmg_qh_qonly.events.jsonl",
        name="Stored event labels")
    if require_stored_labels and (stored_event_labels != event_labels
                                  or canonical_hash(stored_event_labels) != canonical_hash(event_labels)):
        raise RuntimeError("Stored event labels differ from authoritative GT regeneration.")
    governance = governance_metrics(
...
def _authoritative_sequence_summary(root, sequence, config, support_iou, dataset_root):
    if not completed_evaluated_sequence(
            root, sequence, support_iou, dataset_root=dataset_root, config=config):
        raise RuntimeError("Evaluation is incomplete or corrupt for {!r}.".format(sequence))
    sequence_root = safe_sequence_dir(root, sequence, create=False)
    summary = _require_canonical_json_file(
        sequence_root / "sequence_summary.json", name="Sequence summary")
    _exact_keys(summary, _SUMMARY_KEYS, "Sequence summary")
    expected = _recompute_authoritative_sequence_summary(
        root, sequence, config, support_iou, dataset_root, require_stored_labels=True)
    if summary != expected or canonical_hash(summary) != canonical_hash(expected):
        raise RuntimeError(
            "Sequence summary does not equal authoritative GT/trace recomputation.")
    return summary
...
def validate_evaluated_root(root, scope, sequences, support_iou, config,
    ...
    dataset_root = metadata["identity"]["dataset_root"]
    summaries = [_authoritative_sequence_summary(
        root, sequence, config, support_iou, dataset_root)
...
def summarize_evaluation(args, config, sequences, metadata):
    ...
    if args.scope == "internal42" and gate["pass"]:
        ...
        lock = {
            "schema_version": SCHEMA_VERSION,
            "kind": "final_stage1_quarantine_v3a_policy",
            "support_iou": float(args.support_iou),
            ...
            "protocol_manifest": provisional["protocol_manifest"],
            "feature_schema_hash": provisional["feature_schema_hash"],
            "selection_audit": provisional["selection_audit"],
            "bootstrap_stability": provisional["bootstrap_stability"],
            "logo_stability": provisional["logo_stability"],
        ...
        write_json(Path(args.output_dir) / "final_policy_lock_v3a.json", lock)
    if args.scope == "confirm62" and len(sequences) == 62 and gate["pass"]:
        ...
        unlock = {
            "schema_version": SCHEMA_VERSION,
            "kind": "rmg_stage1_v3a_val47_unlock",
        ...
        write_json(Path(args.output_dir) / "val47_unlock_v3a.json", unlock)
    return aggregate, gate


def _dataset_manifest_hash(dataset_root, sequences):
    """Seal authoritative GT and every ordered RGB/TIR image byte."""
    supplied_root = Path(dataset_root)
    if not supplied_root.is_absolute():
        raise RuntimeError("Authoritative dataset_root must be absolute.")
    root = supplied_root.resolve()
    if not root.is_dir():
        raise RuntimeError("Authoritative dataset_root does not exist: {}".format(root))
    entries = []
    for sequence in sequences:
        if not isinstance(sequence, str) or not _SAFE_SEQUENCE.fullmatch(sequence):
            raise RuntimeError("Dataset manifest contains an unsafe sequence name.")
        sequence_root = root / sequence
        if not sequence_root.is_dir():
            raise RuntimeError("Authoritative dataset sequence is missing: {}".format(sequence))
        entry = {"sequence": sequence, "ground_truth": {}}
        for annotation in ("visible.txt", "infrared.txt"):
            path = sequence_root / annotation
            digest = file_sha256(path)
            if digest is None:
                raise RuntimeError("Authoritative GT file is missing: {}".format(path))
            entry["ground_truth"][annotation] = {
                "size": int(path.stat().st_size), "sha256": digest,
            }
        for modality in ("visible", "infrared"):
            directory = sequence_root / modality
            if not directory.is_dir():
                raise RuntimeError("Authoritative image directory is missing: {}".format(directory))
            files = sorted(path for path in directory.iterdir()
                           if path.is_file() and path.suffix.lower() == ".jpg")
            ordered = [{"name": path.name, "size": int(path.stat().st_size),
                        "sha256": file_sha256(path)} for path in files]
            entry[modality] = {"count": len(ordered), "ordered_files": ordered}
        if entry["visible"]["count"] <= 0 or (
                entry["visible"]["count"] != entry["infrared"]["count"]):
            raise RuntimeError("Authoritative sequence has unaligned image counts: {}".format(
                sequence))
...
def _resolve_support_iou(args, config, parent_hash, design_sequences=None,
    ...
    required_kind = ("provisional_stage1_quarantine_v3a_policy"
                     ...
                     else "final_stage1_quarantine_v3a_policy")
...
def run_select_phase(args, config, sequences, provenance):
    ...
    for root in roots:
        ...
        aggregate_candidate = dict(aggregate)
        aggregate_candidate["support_threshold"] = threshold
    ...
    passed = (selected is not None and stability["selected_support_threshold"] == selected
              and stability["bootstrap"]["modal_support_threshold"] == selected
              and stability["bootstrap"]["selected_threshold_match_fraction"]
              >= stability_config["selected_threshold_match_fraction_min"]
              and stability["leave_one_group_out_same_non_none_threshold_count"]
              >= stability_config["leave_one_group_out"][
                  "same_non_none_threshold_min"])
    ...
    feature_schema = {
        "schema_version": SCHEMA_VERSION,
        "arms": list(ARMS),
        "controller_observation_keys": sorted(
            __import__("analysis.memory_oracle.quarantine_controller_v3a",
                       fromlist=["ALLOWED_OBSERVATION_KEYS"]).ALLOWED_OBSERVATION_KEYS),
        "online_actions": ["skip", "quarantine", "release", "discard"],
        "probe_offsets": list(config["quarantine"]["probe_offsets"]),
    }
    feature_schema_hash = canonical_hash(feature_schema)
    _atomic_text(output / "feature_schema_hash_v3a.txt", feature_schema_hash + "\n")
    selection_audit = {
        "schema_version": SCHEMA_VERSION, "kind": "rmg_stage1_v3a_selection_audit",
        "pass": passed, "selected_support_threshold": selected,
        "selection": selection_gate, "candidates": artifact_candidates,
        "config_hash": canonical_hash(config),
        "parent_provenance_hash": provenance["content_hash"],
    }
    selection_audit["content_hash"] = canonical_hash(selection_audit)
    bootstrap_audit = {
        "schema_version": SCHEMA_VERSION,
        "kind": "rmg_stage1_v3a_bootstrap_stability",
        "pass": passed, "selected_support_threshold": selected,
        "bootstrap": stability["bootstrap"],
        "stability_content_hash": stability["content_hash"],
    }
    bootstrap_audit["content_hash"] = canonical_hash(bootstrap_audit)
    logo_audit = {
        "schema_version": SCHEMA_VERSION, "kind": "rmg_stage1_v3a_logo_stability",
        "pass": passed, "selected_support_threshold": selected,
        "hash_groups": stability["hash_groups"],
        "leave_one_group_out": stability["leave_one_group_out"],
        "same_non_none_threshold_count": stability[
            "leave_one_group_out_same_non_none_threshold_count"],
        "stability_content_hash": stability["content_hash"],
    }
    logo_audit["content_hash"] = canonical_hash(logo_audit)
    manifest = {
        "schema_version": SCHEMA_VERSION, "kind": "rmg_stage1_v3a_protocol_manifest",
        "pass": passed, "scope": "design83", "sequences": list(sequences),
        "sequences_hash": canonical_hash(list(sequences)),
        "support_threshold_grid": candidates,
        "config_hash": canonical_hash(config),
        "source_sha256": {name: file_sha256(path) for name, path in IDENTITY_SOURCES.items()},
        "parent_provenance_hash": provenance["content_hash"],
        "feature_schema_hash": feature_schema_hash,
        "selection_audit_content_hash": selection_audit["content_hash"],
        "bootstrap_stability_content_hash": bootstrap_audit["content_hash"],
        "logo_stability_content_hash": logo_audit["content_hash"],
    }
    manifest["content_hash"] = canonical_hash(manifest)
    write_json(output / "selection_audit_v3a.json", selection_audit)
    write_json(output / "bootstrap_stability_v3a.json", bootstrap_audit)
    write_json(output / "logo_stability_v3a.json", logo_audit)
    write_json(output / "protocol_manifest_v3a.json", manifest)
    lock_path = output / "provisional_policy_lock_v3a.json"
    if not passed:
        if lock_path.exists():
            raise RuntimeError(
                "Failing selection found an existing provisional lock; refusing to mutate it.")
        return None
    lock = {
        "schema_version": SCHEMA_VERSION,
        "kind": "provisional_stage1_quarantine_v3a_policy",
        ...
        "protocol_manifest": _artifact_reference(output / "protocol_manifest_v3a.json")[0],
        "feature_schema_hash": feature_schema_hash,
        "selection_audit": _artifact_reference(output / "selection_audit_v3a.json")[0],
        "bootstrap_stability": _artifact_reference(output / "bootstrap_stability_v3a.json")[0],
        "logo_stability": _artifact_reference(output / "logo_stability_v3a.json")[0],
        "config_hash": canonical_hash(config),
        "parent_provenance_hash": provenance["content_hash"],
...
def _build_identity(args, config, sequences, support_iou, provenance, policy_lock):
    ...
    identity = {
        ...
        "image_manifest_hash": _dataset_manifest_hash(
            str(Path(args.dataset_root).resolve()), sequences),
        "support_iou": float(support_iou),
...
def validate_or_write_metadata(path, identity):
    ...
    if path.is_file():
        ...
        return metadata
    metadata = {
        "schema_version": SCHEMA_VERSION, "kind": "stage1_quarantine_v3a_run_metadata",
        "identity": identity,
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Run the fail-closed RMG-Q Stage 1 v3a protocol artifact chain.")
    ...
    parser.add_argument("--stage1-v2-parent", required=True,
                        help="Frozen v2 design base containing five evaluated roots and No-Go audits.")
...
def main(argv=None):
    ...
    provenance = validate_parent_provenance({
        "stage0_dir": args.stage0_parent,
        "stage1_v1_dir": args.stage1_v1_parent,
        "stage1_v2_dir": args.stage1_v2_parent,
    }, config)
    ...
    if args.phase == "select":
        ...
        print("Stage 1 v3a phase 'select' complete: {}".format(Path(args.output_dir).resolve()))
        return
    if args.design_root:
    ...
    print("Stage 1 v3a phase {!r} complete: {}".format(args.phase, output))
